...
public class ReservationService
{
    ...
    public  static void insertReservation(Reservation reservation)throws Exception {
        try (Connection connection = DatabaseConnection.getConnection()) {
            ...
            java.sql.Date auj=new java.sql.Date(now.getTime());
            HeureAvantApresRes heureAvantApresRes = HeureAvantApresRes.read(connection);
            AgeEnfantMaxRemise enfanMaxAge=AgeEnfantMaxRemise.getLastByDate(connection,auj);
            reservation.setIdEnfantAgeMax(enfanMaxAge.getId());
            Vol vol = Vol.getById(connection, reservation.getIdVol());
            ...
            if (reservationTime <= timeMax) {
                ...
                double prixReservation = volPrixTypeSiege.getPrixUnitaire() * reservation.getNbPlace()+volPrixTypeSiege.getPrixUnitaire() * reservation.getPlaceEnfant()-volPrixTypeSiege.getPrixUnitaire() * reservation.getPlaceEnfant()*enfanMaxAge.getRemise()/100;
