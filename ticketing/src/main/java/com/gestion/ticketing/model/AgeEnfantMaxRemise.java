package com.gestion.ticketing.model;
import java.sql.*;
...
public class AgeEnfantMaxRemise {
    ...
    public static AgeEnfantMaxRemise getLastByDate(Connection connection) throws SQLException {
        String sql = "SELECT * FROM age_enfant_max_remise ORDER BY date_ens DESC LIMIT 1";
        try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            if (rs.next()) {
                return mapResultSetToAgeEnfantMaxRemise(rs);
            }
